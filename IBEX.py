...
# Display moving average metrics
...
def process_price_data(df): df['Return'] = (1 + df['Close'].pct_change()).cumprod() - 1 df['Return'] = df['Return'].fillna(0) for ma in [50, 100, 150, 200]: df[f'{ma}MA'] = df['Close'].rolling(window=ma).mean() return df

Calculate RSI

def calculate_rsi(df, window=14): delta = df['Close'].diff() gain = (delta.where(delta > 0, 0)).rolling(window=window).mean() loss = (-delta.where(delta < 0, 0)).rolling(window=window).mean() rs = gain / loss df['RSI'] = 100 - (100 / (1 + rs)) return df

Calculate MACD

def calculate_macd(df): exp1 = df['Close'].ewm(span=12, adjust=False).mean() exp2 = df['Close'].ewm(span=26, adjust=False).mean() df['MACD'] = exp1 - exp2 df['Signal'] = df['MACD'].ewm(span=9, adjust=False).mean() return df

Display moving average metrics

def display_moving_averages(df): cols = st.columns(3) for i, ma in enumerate([200, 150, 50]): if len(df) >= ma: value = df[f'{ma}MA'].iloc[-1] with cols[i]: st.subheader(f"Moving Average {ma}") st.metric(label=f"MA{ma}", value=f"{value:.2f}") else: with cols[i]: st.warning(f"Not enough data for MA{ma}")

Main app function

def app(): st.markdown("<style>main {background-color: #f5f5f5;}</style>", unsafe_allow_html=True) st.title("IBEX 35 Analysis") st.markdown("Visualize price history, returns, and fundamental ratios from Yahoo Finance.") st.markdown("## Disclaimer") st.info("This application is for educational purposes only. Created by Javier Audibert.")
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Charts", "Fundamentals", "Company Info", "Technical Indicators", "Risk"])
...
with tab4:
    st.header("RSI and MACD")
    st.line_chart(price_df[['RSI']].dropna())
    st.line_chart(price_df[['MACD', 'Signal']].dropna())
